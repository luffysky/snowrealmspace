...
// href 是相對後台根的子路徑；渲染時前綴 ADMIN_BASE（可能含隨機碼）
const SECTIONS: { group: string; items: { href: string; label: string; desc: string }[] }[] = [
  {
    group: 'AI',
    items: [
      { href: '/ai-keys', label: 'AI 金鑰', desc: '各家 provider 金鑰（加密存 DB）' },
      { href: '/ai/models', label: 'AI 模型', desc: '啟用/免費/串流·工具·視覺標記' },
      { href: '/ai/candidates', label: '候選鏈', desc: '各用途主模型與 fallback 順序' },
      { href: '/ai/usage', label: 'AI 用量與成本', desc: '免費vs付費、升級/fallback/cache 率' },
      { href: '/ai/quota', label: '每日額度', desc: '各 space 每日呼叫計數' },
      { href: '/ai/cache', label: '回應快取', desc: '命中率與 scope（跨 space 隔離）' },
    ],
  },
  {
    group: 'Agent 與內容',
    items: [
      ...
      { href: '/flags', label: 'Feature Flags', desc: '全域旗標即時切換 + space 覆寫' },
    ],
  },
  {
    group: '系統與稽核',
