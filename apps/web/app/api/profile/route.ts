...
const schema = z
  .object({ avatarAssetId: z.string().uuid().nullable() })
  .strict()

/**
 * 更新使用者大頭貼（profiles.avatar_asset_id）。
 *
 * 位元組只在 assets（ADR-005）—— 這裡只綁 asset id。傳 null 代表移除頭貼。
...
export const PATCH = handler(async (request: NextRequest) => {
  ...
  const { error } = await db
    .from('profiles')
    .update({ avatar_asset_id: parsed.data.avatarAssetId } as never)
    .eq('id', user.id)
  if (error) return fail('INTERNAL', '更新失敗。')

  return ok({ avatarAssetId: parsed.data.avatarAssetId })
})
