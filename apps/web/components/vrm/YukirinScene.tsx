...
function detectTier(): QualityTier {
  ...
  if (!gl) return 'low'
  // 低階 Android 降級，iPhone / 現代 Android 維持 high
...
function setupLights(scene: THREE.Scene) {
  const ambient = new THREE.AmbientLight(0xd0c0f8, 0.8)
  scene.add(ambient)

  const keyLight = new THREE.DirectionalLight(0xf0e8ff, 1.2)
  keyLight.position.set(1, 2, 3)
  scene.add(keyLight)

  const fillLight = new THREE.DirectionalLight(0xffd0d0, 0.6)
  fillLight.position.set(-2, 1, 1)
  scene.add(fillLight)

  const rimLight = new THREE.DirectionalLight(0xd0d0ff, 0.4)
  rimLight.position.set(0, 3, -2)
  scene.add(rimLight)
}

// 玻璃球粒子背景（High 模式：6 顆球 + 200 粒子）
...
interface YukirinSceneProps {
  ...
  allowOrbit?: boolean
  /** 效能模式：high = 全效能（預設）/ low = 省電（低 DPR、無背景粒子、無閒置動畫） */
  quality?: 'high' | 'low'
  /** FPS 持續低於 20 時觸發（最多一次，可用來主動關心使用者）*/
  onLowPerformance?: () => void
  /** 外部注入的 mood 變更（從 AI Guide 觸發） */
  onLoad?: (controls: YukirinControls) => void
  /** 角色大小倍率（預設 1.0，範圍 0.5~2.0） */
  characterSize?: number
  className?: string
}

export interface YukirinControls {
  setMood: (mood: YukirinMood, durationMs?: number) => void
  playAnimation: (name: string) => void
  /** 取得目前正在播放的動作名稱 */
  getCurrentAnimation: () => string
  /** 取得目前心情 */
  getCurrentMood: () => YukirinMood
  /** 讓雪凜朗讀文字並同步口型（需要瀏覽器 TTS 支援） */
...
// 表情對應 VRM Expression 名稱
const MOOD_EXPRESSION: Record<YukirinMood, string> = {
  happy:     'happy',
  surprised: 'surprised',
  relaxed:   'relaxed',
  neutral:   'neutral',
...
export default function YukirinScene({
  cameraMode = 'fullBody',
  mood = 'happy',
  showBackground = true,
  allowOrbit = false,
  ...
  const currentMoodRef = useRef<YukirinMood>(mood)
  const currentAnimationRef = useRef<string>('idle')
  const blinkTimerRef = useRef(0)
  const blinkIntervalRef = useRef(3 + Math.random() * 4) // 3-7 秒
  ...
  // 用 ref 存 callback 避免加入 effect 依賴
  ...
  const baseScaleRef      = useRef<number>(1)   // 自動縮放後的基礎 scale
  ...
  const pendingFbxAnimRef = useRef<string>('fbx_breathing') // FBX 還未載入時暫存請求
  const userSelectedPoseRef = useRef<string | null>(null) // 使用者主動選的姿勢，setMood 不覆蓋
  const waveAnimActiveRef   = useRef(false) // 防止 wave 動畫並行
  const allowOrbitRef  = useRef(allowOrbit)
  allowOrbitRef.current = allowOrbit

  // 點擊互動：隨機切換表情
  const CLICK_REACTIONS: YukirinMood[] = ['surprised', 'wink', 'excited', 'happy', 'relaxed']
  ...
  const playArmWave = useCallback((vrm: VRM) => {
    if (waveAnimActiveRef.current) return // 防並行
    ...
    if (!upperArm) return
    waveAnimActiveRef.current = true
    let t = 0
    let rafId = 0
    const reset = () => {
      upperArm.rotation.set(0, 0, 0)
      if (lowerArm) lowerArm.rotation.set(0, 0, 0)
      waveAnimActiveRef.current = false
    }
    const anim = () => {
      if (!mountedRef.current) { reset(); return }
      t += 0.07
      // 縮小幅度：-0.5 ± 0.2，讓手臂輕揮而非完全橫展
      upperArm.rotation.z = -0.5 + Math.sin(t * 3) * 0.2
      upperArm.rotation.x = -0.05
      if (lowerArm) lowerArm.rotation.z = 0.1 + Math.sin(t * 3 + 0.5) * 0.1
      if (t < Math.PI * 2.5) { rafId = requestAnimationFrame(anim) }
      else { reset() }
    }
    rafId = requestAnimationFrame(anim)
    // 保底：4 秒後強制 reset，避免 rAF 卡住
    setTimeout(() => { cancelAnimationFrame(rafId); reset() }, 4000)
  }, [])
  ...
  // 舞步擺動（靠 targetBodyYRot 驅動，安全不衝突）
  ...
  /** 根據心情切換 FBX idle 動畫（若使用者已手動選姿勢則不覆蓋） */
  const switchIdleByMood = useCallback((newMood: YukirinMood) => {
    ...
    // orbit / fullBody 模式只用站立 breathing，避免坐姿讓腿飄空
    const clipName = allowOrbitRef.current ? 'fbx_breathing' :
      (newMood === 'happy' || newMood === 'excited') ? 'fbx_happy' :
  ...
  // 平滑過渡表情
  const transitionExpression = useCallback((vrm: VRM, target: YukirinMood, durationMs = 300) => {
  ...
  useEffect(() => {
    ...
    if (tier === 'low') return // low tier 由父層顯示 PNG fallback
    ...
    loader.loadAsync('/models/Yukirin.vrm').then((gltf) => {
      const vrm: VRM = gltf.userData.vrm
      VRMUtils.removeUnnecessaryJoints(vrm.scene)
      // VRM 模型預設面向 -Z（與 Three.js 相機方向一致），不需旋轉
      vrm.scene.rotation.y = 0

      // 自動縮放：把模型高度撐到約 1.6m 單位
          ...
          playAnimation: (name) => {
            ...
            switch (name) {
              ...
              case 'fbx_sitting': {
                ...
                if (!clip) {
                  // FBX 還沒載入完，先存起來等 Promise.all.then() 播放
      ...
      // 隨機 idle 動作排程（orbit 模式只用站立動作，避免坐姿讓腿飄空）
      const idleActions = allowOrbit ? [
        'look_left', 'look_right', 'idle',
        'nod', 'tilt_head',
      ] as const : [
      ...
      console.error('[YukirinScene] VRM 載入失敗', err)
    })
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        // 2 秒後恢復 happy
        setTimeout(() => {
          transitionExpression(vrm, 'happy', 400)
    ...
    // ── Orbit 互動（allowOrbit 模式，掛在 canvas 上）────────────────────────
    ...
    if (allowOrbit) {
      ...
      const onCanvasDown = (e: PointerEvent) => {
        ...
        // document-level 追蹤，指針離開 canvas 也不斷
    ...
    const animate = () => {
      ...
      // 眼睛跟隨滑鼠（用 lookAt position 而非 target 物件）
      if (vrm?.lookAt) {
