...
test.describe('帳號密碼註冊 / 登入', () => {
  test('註冊 → 進站 → 引導綁定頁', async ({ page }) => {
    ...
    await page.locator('#pw-password').fill('supersecret123')
  ...
  test('重複 email 註冊會被擋', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '註冊', exact: true }).click()
    await page.locator('#pw-email').fill(email)
    await page.locator('#pw-password').fill('anotherpassword')
    await page.locator('form:has(#pw-password) button[type="submit"]').click()
  ...
  test('密碼太短擋在前端', async ({ page }) => {
    await page.goto('/login')
    await page.getByRole('button', { name: '註冊', exact: true }).click()
    await expect(page.locator('#pw-password')).toHaveAttribute('minlength', '8')
  })
})
