...
- [~] **#55** 內建背景擴充 + 套件化下載（動態/靜態/Lottie，分類、下載後套用、未下載可預覽）。
  - [x] ~~程序化場景擴充到 **每類 50 = 300**（6 類：天氣/星空宇宙/自然/慶祝/簡約/**城市夜景**新增）~~ ✅ 0729
  - [x] ~~**裝飾品 widget**（81 Fluent Emoji、自由擺放、大小/旋轉/透明度/漸層染色、0060+RLS、overlay 不擋點擊）~~ ✅ 0729
  - [ ] **免費商用 Lottie 背景** + **動漫（第 7 類，插畫/Lottie）各 50 → 共 350**：卡 LottieFiles 下載流程 + 逐一挑構圖/keyframe。
  - [ ] 套件化下載（下載後套用、未下載可預覽）尚未做。
- [ ] **裝飾品擴充**（Luffy 0729 回饋：現在 81 個太少、且太多是「表情臉」）：
  - [ ] 大幅增量、**減少表情臉**、加更多動物/植物/食物/物件/節慶/自然小物（維持免費可商用，Fluent Emoji 之外可找 OpenMoji(CC-BY)/Twemoji(CC-BY) 等）。
  - [ ] **Lottie 動態裝飾**：找免費可商用的小動畫（愛心、星星、蝴蝶、閃粉…）當可擺放的動態裝飾（跟天氣 Lottie 同機制，走下載流程 + 逐一挑）。
- [ ] 對話歷史摘要（長對話壓縮）。
