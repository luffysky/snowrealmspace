...
> ⚠️ **這份總覽已過時（建立於 2026-07-23）。** 之後的工作階段已實作了 C（C1–C7）、
> D（AI Core 路由/Agent/記憶）、E（cron 掃描）、F（骨架）與隱私刪除的一大部分。
> 下表的「未開始 0%」不再反映現況；動工前請以程式碼與各 Milestone 的 verify 腳本為準，
> 不要以這張表為準。（待完整重新盤點。）
...
| 刪除 space（7 天寬限、R2 先於 DB） | ✅ 危險區域 UI + space-purge job + 還原（0028-0030）。附帶修掉 activity_events cascade 潛在 bug |
| 刪除帳號（auth.users + 名下 space） | ⬜ **未做** |
| 帳號匯出（zip） | ⬜ **未做** |
| AI 資料聲明頁 | ✅ `settings/ai-data` |
| 資料地圖頁 | ✅ `settings/data` |
| `asset.purge` GC job | ✅ `handleStorageGc` 已含 30 天後永久刪除（R2 先於 DB） |
...
| 項目 | 狀態 | 備註 |
|---|---|---|
| CI 實際執行 | ✅ | remote 已接上，workflow 會跑 |
| **CI 改寫為 Zeabur** | 🚧 | Vercel 假設已移除，仍在修 e2e job |
| Cron 機制 | ✅ | pg-boss `schedule()`，見 `apps/worker/src/schedules.ts` |
| Dockerfile（web / worker） | ✅ | build context 為 repo 根目錄 |
| 部署手冊 | ✅ | `14-deploy-zeabur.md` |
| `storage.gc` job | ✅ | `handlers/maintenance.ts`，每日 03:00 UTC |
| `queue-health` 檢查 | ✅ | 同上，每 5 分鐘 |
| Sentry / 監控 | ⬜ |  |
