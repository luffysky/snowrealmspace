...
**修訂（2026-07-24，使用者指示）**
原本有「未達閉環的 Milestone 不得開始下一個 Milestone」的硬性順序限制，
**現已取消**。使用者要求趕進度，允許多個 Milestone 平行開發。

各 Milestone 的閉環驗收條件（`10-acceptance.md`）**仍然有效** ——
差別只在「什麼時候驗收」：不再是「進下一個之前」，而是「上線之前」。
也就是說可以先把 C/D 的程式碼寫好，B 的 Q10 手動走查留到最後一起做。

風險（已知並接受）：平行開發時，前一個 Milestone 若有設計缺陷，
會同時影響後面已寫好的部分。緩解方式是每個 Milestone 的自動化閘門
（型別、測試、RLS、E2E）照舊全綠才提交，把「未驗收」的範圍
限縮在「需要人操作」的那幾項。
