...
**修訂（2026-07-24，使用者指示）：** 原規定「由專案負責人親筆撰寫，不由 AI 生成」。
使用者趕時間，明確授權**由 AI 代寫全部 E 內容**（語錄、提示、問候、驚喜、生日信）。

代寫時的品質底線（等同人工撰寫的要求）：
- 全繁體中文，語氣一致（溫柔、不說教、不空泛）
- 通過 §9 的 `FORBIDDEN_PATTERNS`（不假裝是真人、不宣稱記得不存在的事）
- 生日信仍走生日鏈結構，但內容由 AI 生成

### 7.2 內容規模：至少 10 年份量

**修訂（2026-07-24，使用者指示）：** §3 表格的 Alpha 最低數量（quote 60、prompt 80…）
**太少**。這是每天要用的產品，使用者要求準備**至少 10 年份量**。

10 年 ≈ 3650 天。設計上不必每一則都全新（那會犧牲品質），而是靠：
- **大型固定池** + **冷卻窗**（quote 60 天、prompt 90 天不重複）
- **脈絡選取**（時段、季節、星期、心情、距註冊天數）讓同一則在不同情境有不同意義
- 池規模 × 冷卻窗 × 脈絡維度的組合，讓「感覺重複」的間隔遠長於 10 年

實際數量以 `content/` 下的資料檔為準，seed 腳本會統計並在低於門檻時警告。
