...
import { createSnapshotFromAsset } from './snapshots'
import { type ProviderKey, decryptToken, encryptToken, providerConfig, refreshToken } from './providers'

/**
 * Milestone F — sync 核心（S1 抽出，web 與 worker 共用；worker 情境用 service role client）。
 *
...
/**
 * 取得可用的 access token。過期且有 refresh token → 先 refresh，加密回存 design_connections。
 * 任一步失敗都回明確錯誤（不靜默）。db 由呼叫端提供（web：owner RLS；worker：service role）。
 */
 ...
 */
async function storeRenditionAsset(ctx: SyncContext, url: string, filename: string): Promise<AssetResult> {
  let res: Response
  try {
    res = await fetch(url)
  } catch (err) {
  ...
  // 新建 asset：先 pending 拿 id，上傳位元組，再標 ready
...
/**
 * 同步單一檔案：抓取 → upsert design_file → 存預覽 asset → 建 snapshot。
 *
 * **不吞 ProviderApiError**（adapter.fetchFile 會拋）——讓呼叫端（worker）據 status 決定
 * 重試/退避/放棄。本地錯誤（下載/存檔/建版本）不拋，回 status:'error' 並落 last_error。
 */
export async function syncOneFile(
  ...
  const fetched = await adapter.fetchFile(accessToken, externalId) // 可能拋 ProviderApiError

  const upserted = await upsertDesignFile(ctx, conn.id, provider, fetched)
  if (!upserted) {
    return { externalId, designFileId: null, snapshotId: null, status: 'error', message: '建立作品記錄失敗。' }
  }
  ...
  if (!asset.ok) {
    ...
    return { externalId, designFileId: upserted.fileId, snapshotId: null, status: 'error', message: asset.error }
  }
  ...
  if (!snap.ok) {
    ...
    return { externalId, designFileId: upserted.fileId, snapshotId: null, status: 'error', message: msg }
  }
...
/**
 * 同步一批明確挑選的檔案（禁「同步全部」——由呼叫端傳入清單）。逐檔獨立，
 * **吞** ProviderApiError 成 status:'error'，適合需要一次回報整批結果的同步情境。
 * worker 走 job 化時用 syncOneFile（不吞）以便分流重試。
 */
export async function syncSelectedFiles(
  ...
  for (const externalId of externalIds) {
    try {
      results.push(await syncOneFile(ctx, conn, accessToken, externalId))
    } catch (err) {
      const message =
