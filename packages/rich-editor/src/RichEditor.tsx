...
export function RichEditor({
  ...
  placeholder?: string
  /**
   * 詢問使用者一個網址（連結／圖片）。由宿主 app 注入自己的對話框，
   * 讓 SDK 不依賴任何特定 UI；回傳 null 代表取消。
   */
  promptLink: (message: string) => Promise<string | null>
  /** GIF 搜尋端點（宿主 app 的代理），預設 `/api/giphy`。 */
      ...
      <div className="sr-rich-toolbar">
        ...
        <B
          ...
          on={async () => {
            ...
            const url = await promptLink('連結網址')
        ...
        <B
          ...
          t="圖片網址"
          on={async () => {
            const url = await promptLink('圖片網址')
        ...
        <GifPicker giphyEndpoint={giphyEndpoint} onSelect={(url) => editor.chain().focus().setImage({ src: url }).run()} />
      </div>
