...
// 平台 gateway 逾時偏短（實測 4s×2=8s 仍被判 502），故壓到 2s（總計最壞 ~4s），
// 保證路由一定在 gateway 逾時前回應：連得到就成功，連不到就回可讀的 PROVIDER_ERROR
// （widget 會顯示原因），不再出現 gateway 502。Open-Meteo 正常 < 1s，2s 對「連得到」已足夠。
